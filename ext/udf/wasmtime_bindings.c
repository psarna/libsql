...
void run_wasm(sqlite3_context *context, int argc, sqlite3_value **argv) {
  ...
  wasm_engine_t *engine = (wasm_engine_t *)context->pVdbe->db->wasm.engine;
  assert(engine);
...
FuncDef *try_instantiate_wasm_function(sqlite3 *db, const char *pName, int nName, const char *pSrcBody, int nBody, int nArg, char **err_msg_buf) {
  ...
  if (!db->wasm.engine) {
    db->wasm.engine = (libsql_wasm_engine_t*)wasm_engine_new();
  }
    ...
    if (error) {
      ...
      if (error) {
        if (err_msg_buf) {
          ...
          *err_msg_buf = sqlite3DbStrNDup(db, err_str.data, err_str.size);
  ...
  error = wasmtime_module_new((wasm_engine_t*)db->wasm.engine, (uint8_t *)compiled_wasm.data, compiled_wasm.size, &module);
  if (!source_already_compiled) {
