...
fn maybe_set_err_buf(
    ...
    if !err_buf.is_null() {
        ...
        unsafe { *err_buf = err_ptr as *const c_char };
    }
}
...
pub fn libsql_compile_wasm_module(
    ...
    let compiled_owned = if source_already_compiled {
        ...
        let src_body_str: &str = match std::str::from_utf8(src_body) {
            Ok(src) => src,
            Err(e) => {
                maybe_set_err_buf(err_msg_buf, format!("{}", e), alloc_err);
        ...
        match wat::parse_str(src_body_str) {
            ...
            Err(_) => {
                ...
                match wat::parse_str(&src_body_dequoted[1..src_body_dequoted.len() - 2]) {
                    Ok(src) => src,
                    Err(e) => {
                        maybe_set_err_buf(err_msg_buf, format!("{}", e), alloc_err);
                        return std::ptr::null() as *const c_void;
...
pub struct libsql_wasm_udf_api {
    ...
    libsql_value_text: unsafe extern "C" fn(*const c_void) -> *const c_char,
    libsql_value_blob: unsafe extern "C" fn(*const c_void) -> *const c_void,
    libsql_value_bytes: unsafe extern "C" fn(*const c_void) -> i32,
    libsql_result_error: unsafe extern "C" fn(*const c_void, *const c_char, i32),
    ...
    libsql_result_text: unsafe extern "C" fn(*const c_void, *const c_char, i32, *const c_void),
...
pub fn libsql_run_wasm(
    ...
    let func_name: &str = match unsafe { CStr::from_ptr(func_name) }.to_str() {
        Ok(s) => s,
        Err(e) => {
    ...
    let memory = match instance.get_memory(&mut store, "memory") {
        ...
        None => {
            unsafe {
                ((*api).libsql_result_error)(
                    libsql_ctx,
                    "Memory \"memory\" not found in wasm module".as_ptr() as *const c_char,
                    -1,
                );
            }
            return;
    ...
    for i in 0..argc {
        ...
        match unsafe { ((*api).libsql_value_type)(arg) } as i8 {
            ...
            LIBSQL_TEXT => {
                let text: &str = unsafe {
                    match CStr::from_ptr(((*api).libsql_value_text)(arg)).to_str() {
                        Ok(s) => s,
                        Err(e) => {
                            let err = format!("Function name is not valid utf-8: {}", e);
                            ((*api).libsql_result_error)(
            ...
            _ => {
                unsafe {
                    ((*api).libsql_result_error)(
                        libsql_ctx,
                        "Unknown libSQL type".as_ptr() as *const c_char,
                        19,
                    )
                }
                return;
    ...
    match result {
        ...
        Val::I32(v) => {
            ...
            match memory.data(&store)[v] as i8 {
                ...
                _ => unsafe {
                    ((*api).libsql_result_error)(
                        libsql_ctx,
                        "Malformed result type byte".as_ptr() as *const c_char,
                        26,
                    )
                },
            }
        ...
        _ => unsafe {
            ((*api).libsql_result_error)(
                libsql_ctx,
                "Malformed result type".as_ptr() as *const c_char,
                21,
            )
        },
    }
