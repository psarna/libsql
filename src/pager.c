...
static int pager_end_transaction(Pager *pPager, int hasSuper, int bCommit){
  ...
  */
  ...
  if( !pPager->exclusiveMode 
   && (!pagerUseWal(pPager) || pPager->pWalMethods->xExclusiveMode(pPager->pWal, 0))
  ){
...
static int pagerPagecount(Pager *pPager, Pgno *pnPage){
  ...
  */
  ...
  nPage = pPager->pWalMethods->xDbsize(pPager->pWal);
...
int sqlite3PagerOpen(
  ...
  libsql_wal_methods *pWal,/* WAL methods to use */
...
int sqlite3PagerLockingMode(Pager *pPager, int eMode){
  ...
  assert( pPager->exclusiveMode || 0==pPager->pWalMethods->xHeapMemory(pPager->pWal) );
  if( eMode>=0 && !pPager->tempFile && !pPager->pWalMethods->xHeapMemory(pPager->pWal) ){
